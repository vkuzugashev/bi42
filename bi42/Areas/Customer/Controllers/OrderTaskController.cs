...
namespace bi42.Areas.Customer.Controllers
{
    public class OrderTaskController : Controller
    {
        ...
        public ActionResult Index(int orderid, int? page)
        {
            ...
            db.SetLastAccess("cstm_order_task_"+orderid, UserID);
        ...
        public ActionResult Edit(int orderid, int id)
        {
            OrderTask task;
            if (id != 0)
            {
        ...
        public ActionResult Update(int orderid, int id, bool? delAttach, HttpPostedFileBase file)
        {
            OrderTask task;
...
//                task.UpdateTime = DateTime.Now;
                ...
                db.SaveChanges();
                return RedirectToAction("Index", new { orderid = task.OrderID });
            }
            else
            {
                ...
                return RedirectToAction("Edit", new { orderid=task.OrderID, id = task.OrderTaskID });
            }
        }
